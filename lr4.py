...
model.compile(opt.SGD(lr=0.001, momentum=.2), loss='categorical_crossentropy', metrics=['accuracy'])
H = model.fit(train_images, train_labels, epochs=7, batch_size=100, validation_split=0.1)
test_loss, test_acc = model.evaluate(test_images, test_labels)
draw(H, "SGD")
model = build_model()

model.compile(opt.RMSprop(lr=0.001), loss='categorical_crossentropy', metrics=['accuracy'])
H = model.fit(train_images, train_labels, epochs=7, batch_size=100, validation_split=0.1)
test_loss, test_acc = model.evaluate(test_images, test_labels)
draw(H, "RMSprop")