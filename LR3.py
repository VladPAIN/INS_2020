...
class lab3:
    ...
    def find_overfit(self):
        for i in range(3, self.k):
            ...
            partial_train_targets = np.concatenate( [self.train_targets[:i * self.num_val_samples], self.train_targets[(i + 1) * self.num_val_samples:]], axis=0)
            self.model = self.build_model()
            history = self.model.fit(partial_train_data, partial_train_targets, epochs=self.num_epochs, batch_size=1, verbose=0, validation_data=(val_data, val_targets))
    ...
    def fit_model(self):
        ...
        for i in range(self.k):
            ...
            partial_train_targets = np.concatenate([self.train_targets[:i * self.num_val_samples], self.train_targets[(i + 1) * self.num_val_samples:]], axis=0)
            self.model = self.build_model()
            history = self.model.fit(partial_train_data, partial_train_targets, epochs=self.num_epochs, batch_size=1, verbose=0)
            val_mse, val_mae = self.model.evaluate(val_data, val_targets, verbose=0)
            self.all_scores.append(val_mae)
